(function (window)
{
	...
	*/
	...
	p.initialize = function(unit_width_px, unit_height_px, unit_depth_px, savedObject)
	{
		...
		this.isContainer = savedObject.isContainer;
		...
		this.cubeCount_at_y_index = [];
		for (var j = 0; j < this.blockArray3d[0].length; j++)
		{
			this.cubeCount_at_y_index[j] = 0;
			for (var i = 0; i < this.blockArray3d.length; i++)
			{
				for (var k = 0; k < this.blockArray3d[i][j].length; k++)
				{
					if (this.blockArray3d[i][j][k] != "") this.cubeCount_at_y_index[j]++;
				}
			}
		}
...
////////////////////// UTILITY FUNCTIONS FOR THE ARRAY ///////////////////
				...
				for (var j = 0; j < this.blockArray3d[i].length; j++)
				{
					...
					for (var k = 0; k < this.blockArray3d[i][j].length; k++)
					{
						...
						if (this.blockArray3d[i][j][k] != "")
						{
							...
							// back
							if (k == this.array3d_details[i][j].length-1 || this.blockArray3d[i][j][k+1] == "")
							{
		...
		for (var i = left_x; i <= right_x; i++)
		{
			...
			for (var j = top_y; j <= bottom_y; j++)
			{
				...
				for (var k = 0; k < this.blockArray3d[i][j].length; k++)
				{
					if (this.blockArray3d[i][j][k] != "")
					{
						mass += (array3d_details[i][j][k].baseMass + array3d_details[i][j][k].liquidVolume * GLOBAL_PARAMETERS.fluid_density) / array3d_details[i][j][k].volume;
					}
					if (spaces3d[i][j][k] == "B")
					{
						materialSpaces++;
	...
	p.fillWithVolume = function (volume)
	{
		...
		if (GLOBAL_PARAMETERS.fill_spilloff_by_height)
		{
			...
			{
				var total_volume_on_y_index = this.cubeCount_at_y_index[this.y_index] * this.liquid_unit_volume;
				if (total_volume_on_y_index > 0)
				{
					...
					var volume_per_cube = volume_to_distribute_on_y_index / this.cubeCount_at_y_index[this.y_index];
...
////////////////////// DRAWING STUFF /////////////////////////
	p.redraw = function(r)
	{
		...
		if (view_sideAngle < 0)
		{
			for (col = this.rightmostColumn; col >= this.leftmostColumn; col--){colarr[index] = col; index++}
		} else
		{
			for (col = this.leftmostColumn; col <= this.rightmostColumn; col++){colarr[index] = col; index++}
		}

		var rowarr = []; index = 0;
		if (view_topAngle < 0) //90 && rotation < 270)
		{
			for (row = this.highestRow; row <= this.lowestRow; row++){rowarr[index] = row; index++}
		} else
		{
			for (row = this.lowestRow; row >= this.highestRow; row--){rowarr[index] = row; index++}
		}
		...
		for (k = 0; k < this.blockArray3d[0][0].length; k++)
		{
			...
			for (i = 0; i < colarr.length; i++)
			{
				...
				for (j = 0; j < rowarr.length; j++)
				{
					...
					{		
						i_shift = col - this.leftmostColumn;
						j_shift = row - this.highestRow;
						
						// the contents of containers, just the fluid
						...
						else 
						{
							...
							if (array3d_details[col][row][k_rev].right)
							{
								// draw right
								g.setStrokeStyle(thickness);
								g.beginLinearGradientStroke(material.strokeColors, material.strokeRatios, ftr_x, ftr_y, btr_x, ftr_y);
						...
						if (view_sideAngle >= 0 )
						{
							...
							if (array3d_details[col][row][k_rev].back)
							{
								// draw back
								g.setStrokeStyle(thickness);
								g.beginLinearGradientStroke(material.strokeColors, material.strokeRatios, btl_x, btl_y, btr_x, btr_y);
